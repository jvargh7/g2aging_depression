...
mi_dfs <- readRDS(paste0(path_g2a_depression_folder,"/working/g2adhrs_sensitivity multiple imputation.RDS"))
...
for(i in 1:mi_dfs$m){
  df = complete(mi_dfs,action = i) %>% 
    ...
    mutate(duration_dm_ge10 = case_when(duration_dm > 10 ~ 1,
                                        TRUE ~ 0)) %>% 
    left_join(hrs_all_exposure_available %>% 
                dplyr::select(hhidpn,wave,followup),
              by = c("hhidpn","wave"));
...
bind_rows(
  ...
  write_csv(.,"hrs/g2adh04_coefficients for sensitivity poisson regression.csv")
...
bind_rows(
  ...
  write_csv(.,"hrs/g2adh04_contrasts for sensitivity poisson regression with multiple imputation.csv")
