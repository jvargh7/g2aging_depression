...
excluded_df = hrs_all_exposure_available %>% 
...
hrs_sy_cesd_ge3 <- svysummary(hrs_excluded_svy,
                              c_vars = continuous_vars,
                              p_vars = proportion_vars,
                              g_vars = grouped_vars,
                              id_vars = "cesd_ge3"
...
bind_rows(hrs_sy_total %>% mutate(gender = "Total",cesd_ge3 = "Total"),
          ...
          hrs_sy_cesd_ge3 %>% mutate(cesd_ge3 = case_when(cesd_ge3 == 1 ~ "Yes",
                                                          ...
                                                          TRUE ~ NA_character_)) 
          %>% mutate(gender = "Total")) %>% 
  write_csv(.,file = "hrs/g2adh05_excluded characteristics.csv")
